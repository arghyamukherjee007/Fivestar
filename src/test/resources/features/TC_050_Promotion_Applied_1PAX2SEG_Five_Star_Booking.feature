@Regression @P1Scripts @TC_050FivestarBooking
Feature: FiveStar Booking
   'No of Passengers' in Promotions view should be greater than the number of passengers selected in booking

  @TC_050FiveStarBookingScenario1
  Scenario Outline:  '# of Passengers' in Promotions view should be greater than the number of passengers selected in booking

    #Given the required inputs below for "1Pax2Seg" PNR Creation for <TestCase>
    ...
    When User is navigating to "Five Star Admin" first level Tab
		...
		When User is checking for valid "Promo Code" for "Book" flow and update for <TestCase>
		When User is navigating to "Airport List" second level Tab
    ...
    When User is checking for Promotion Amount for "Book" flow for <TestCase>
